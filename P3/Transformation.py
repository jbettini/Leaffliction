...
def show_image_figure(images_to_display):
    s = len(images_to_display)
    if not images_to_display or s != 6:
        raise ValueError(f"Final Fig must contain only 6 image, actually {s}.")
...
def tresh_bin_mask(image, lower_thresh, upper_thresh):
    ...
    binary_mask = cv2.inRange(
        hsv_image,
        np.array(lower_thresh),
        np.array(upper_thresh)
    )
    return binary_mask
...
def pseudolandmarks(image):
    ...
    _, _, _ = pcv.homology.y_axis_pseudolandmarks(img=image, mask=mask)
    left = pcv.outputs.observations['plant']['left_lmk']['value']
    right = pcv.outputs.observations['plant']['right_lmk']['value']
    center = pcv.outputs.observations['plant']['center_h_lmk']['value']
    draw_landmarks_circles(image, left, (0, 0, 255))
    draw_landmarks_circles(image, right, (0, 255, 0))
    draw_landmarks_circles(image, center, (255, 0, 0))
    return image
...
def main():
    try:
        parser = argparse.ArgumentParser(
            description="Process transformation and display an histogram"
        )
        parser.add_argument(
            "--file",
        ...
        if args.file:
            ...
            if osp.exists(args.destination):
                if args.force:
                    ...
                    parser.error(
                        f"{args.destination} exists, use --force to overwrite"
                    )
                    exit(1)
